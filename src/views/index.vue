<template>
  <div class="page-index">
    ...
    <div
      class="demo-list-box"
      :class="{ 'e-unfold': !foldStore[index] }"
      v-for="(demoDataItem, index) in demoData"
      :key="`demoData-${index}`"
    >
      <h4 class="type" @click="toggleFold(index)">
        {{ demoDataItem.typeName }}
        <i class="icon-arrow"></i>
      </h4>
      <ul class="demo-list">
        <li
          class="demo-list-item"
          v-for="(demoItem, demoItemIndex) in demoDataItem.demoList"
...
export default {
  ...
  methods: {
    ...
    handleDemoData () {
      let self = this
      let packagesData = [...config.packagesTypes]
...
.page-index {
  ...
  .header-box {
    ...
    .npm-btn {
      ...
      width: 52px;
      font-size: 0;
      background: url('https://static.npmjs.com/58a19602036db1daee0d7863c94673a4.png')
        no-repeat;
      background-size: 100% 100%;
    }
  }
